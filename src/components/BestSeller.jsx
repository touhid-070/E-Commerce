...
const BestSeller = () => {
  ...
  return (
    <div className='my-10'>   
        ...
        <div className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 dap-y-6'>
            {
                bestSeller.map((item,index)=>(
                    <ProdutItem key={index} id={item._id} name={item.name} image={item.image} price={item.price}/>
                ))
            }
        </div>
    </div>
